from streamlit_calendar import calendar #calendar component mentionned earlier. Found in https://github.com/im-perativa/streamlit-calendar/blob/master/README.md
import streamlit as st
import pandas as pd
from datetime import datetime, timedelta, time
import statistics as stat
from msal import PublicClientApplication #Library for handling authentication with Microsoft Graph API. Handles user login and access tokens for interacting with Microsoft services like Outlook calendars.
import requests #Library to make HTTPS requests to communicate with the Microsoft Graph API
import pytz #Library for handling timezones as they created many issues during the project
import matplotlib.pyplot as plt
import numpy as np
import joblib
from fpdf import FPDF #Library to generate the PDF Report
import os #Used for tempfiles cleanup
import tempfile
import base64 #used for encoding the PDF

# Constants retrieved from the Azure App Registration
CLIENT_ID = '245bdaad-a3d0-44ce-9d77-5fe830189ca1'
AUTHORITY = 'https://login.microsoftonline.com/common'
SCOPES = ['Calendars.ReadWrite', 'Mail.Send']
REDIRECT_URI = 'http://localhost'
LOCAL_TIMEZONE = pytz.timezone("Europe/Zurich")      #We faced issues because all times were shifted to utc. There are multiple manual fixes in the code to account for GMT+1
DAYS = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] #List of days used for the bar chart display

# Initialize session state variables
# Access token will hold the user's authentication token once authenticated.
# Events will store all the calendar events fetched from Microsoft Graph.
...
def authenticate():
    ...
    # Create a PublicClientApplication instance to initiate authentication with Azure AD.
    app = PublicClientApplication(CLIENT_ID, authority=AUTHORITY)
    # Request an interactive authentication flow with the necessary permissions (scopes).
    result = app.acquire_token_interactive(scopes=SCOPES)
    # Return the access token from the result for use in subsequent API calls.
    return result.get('access_token')

def create_calendar_event(access_token, new_event):
    """Create a new calendar event."""
    # API endpoint to create a new event on the user's calendar
    url = 'https://graph.microsoft.com/v1.0/me/events'
    # Authorization headers using the user's access token
...
def convert_to_utc(local_time):
    ...
    # Localize the local time to the set timezone, then convert it to UTC.
...
def fetch_all_calendar_events(access_token, start_date, end_date):
    ...
    # Loop through each calendar and fetch events for the specified date range
    ...
    # Eliminate duplicate events as they mess up calculations
    unique_events = {(e['subject'].strip().lower(), e['start']['dateTime']): e for e in all_events}.values()
    return list(unique_events)

def filter_all_day_events(events):
    """Filter out all-day events from the events list as they are unhelpful for time visualization and make the calendar look ugly"""
    filtered_events = [
...
def tally_hours(events):
    ...
    # Loop through events and categorize based on location
    for event in events:
...
def generate_report(start_date, access_token):
    ...
    for day in week_days:
        ...
        free_hours = max(0, 14 - total_busy_hours) #used the assumption that free time only counts from 08:00 to 22:00, thus a maximum of 14 hours of free time per day
    ...
    # Predict performance and leisure ratings with the pretrained machine learning models. For more info, look at the comments on lines 7-8 of the main page.
    ...
    # All the formatting of the report
    ...
    # Iterate through each day to generate the paragraphs for all 8 days
    for day_data in report_data:
        ...
        # Ratings and Comments decision tree. This uses the ML predictions to issue recommendations.
        ...
        # Add Pie Chart by creating it, then saving it as a pdf and inserting and adjusting its placement in the PDF for each day
    ...
    #Second part of the report using the sleep information inputs to give a Health Assessment
    pdf.set_font("Times", style='B', size=16)
    ...
    # Add table rows. I use the session states to retrieve the useful data input in the first page
    pdf.set_font("Arial", size=10)
    data = [
    ...
    # Check if the chart image exists in session_state and then paste it in the right spot
    if 'chart_image_path' in st.session_state:
        chart_image_path = st.session_state['chart_image_path']
        if os.path.exists(chart_image_path):
            pdf.image(chart_image_path, x=80, y=text_start_y2, w=120)  # Adjusted position and size

    pdf.ln(5)
    pdf.multi_cell(0, 5, st.session_state['deficit_message'])

    # I use the ML prediction about sleep to prepare a decision tree about which advice to give
    ...
    # Save the PDF to a file by first checking and removing an existing file
...
def send_email_via_graph_api(report_file, recipient_email):
    ...
    # Send the email with a POST request
...
#The Calendar Component was taken and simplified to remove unnecessary features like specific location-based daily event displays and redundant visualizations.
#The streamlit-calendar component was developped by im-perativa and retrieved from his github repository: https://github.com/im-perativa/streamlit-calendar/blob/master/README.md
...
# Calendar view selection (predefined parameters in the component)
...
if st.button("Synchronise with Outlook"):
    if st.session_state["access_token"] is None:
        ...
        st.session_state["access_token"] = authenticate()  # Trigger authentication process with the authentication function defined above
...
def main():
    """Main function that controls the app's flow for creating calendar events, tallying hours, and generating reports."""
    ...
    # Check if user is authenticated (access_token exists in session state)
    if "access_token" not in st.session_state:
        ...
        # Create Event that call the create_calendar_event() function
        ...
        # Pie Chart of tallied hours by category (type) that calls the fetch_all_calendar_events() function to count each day. It works in an analogous manner to the tally_hours() function but due to duty separation was not reused here.
            ...
            if "access_token" in st.session_state:
                ...
                if st.button("Calculate Hours"):
                    ...
                    if events:
                        ...
                        total_busy_hours = sum(category_hours.values())
                        total_available_hours = 14 #same problem as above. The max set free daily time is 14 hours. This was already defined in the tally_hours() function but not reused here. But it works.
        ...
        with st.expander("Generate Weekly Reporting"):
            ...
            if st.button("Generate Weekly Report"):
                try:
                    # Generate the report and save it as a file. Here the function is effectively called.
            ...
            # Email Input and Sending Logic with the send_email_via_graph_api() function call.
